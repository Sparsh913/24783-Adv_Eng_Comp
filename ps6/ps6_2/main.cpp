...
// Add a variable to track the last inserted node
...
int main(void)
{
	...
	// Turn on auto-rebalancing before inserting any nodes
	...
	{
		int r=rand()%100;
		lastInsertedNode = tree.Insert(r,0);
	}
	...
	for(;;)
	{
		...
		if(FSKEY_DEL==key && true==pickedNdHd.IsNotNull())
		{
			...
			// If we delete the last inserted node, reset it
			if(pickedNdHd == lastInsertedNode)
			{
				lastInsertedNode = typename BinaryTree<int,int>::NodeHandle();
			}
		}
		...
		// Insert a random number when SPACE key is pressed
		if(FSKEY_SPACE==key)
		{
			int r=rand()%100;
			lastInsertedNode = tree.Insert(r,0);

...
std::vector <PolygonalMesh::VertexHandle> GreedyPathFinder(
		const PolygonalMesh &mesh,
		PolygonalMesh::VertexHandle from,
		PolygonalMesh::VertexHandle to)
{
	std::vector <PolygonalMesh::VertexHandle> path;

	auto vtHd=from;
	path.push_back(vtHd);
	while(vtHd!=to && mesh.NullVertex()!=vtHd)
	{
		PolygonalMesh::VertexHandle nextVtHd=mesh.NullVertex();
		double minDist=(mesh.GetVertexPosition(vtHd)-mesh.GetVertexPosition(to)).GetSquareLength();
		for(auto canVtHd : mesh.GetConnectedVertex(vtHd))
		{
			auto p=mesh.GetVertexPosition(canVtHd);
			auto dist=(p-mesh.GetVertexPosition(to)).GetSquareLength();
			if(dist<minDist)
			{
				nextVtHd=canVtHd;
				minDist=dist;
...
void FloodFill(PolygonalMesh &mesh,PolygonalMesh::PolygonHandle plHd0,YsColor col)
{
	std::vector <PolygonalMesh::PolygonHandle> todo;
	todo.push_back(plHd0);
	mesh.SetColor(plHd0,col);
	while(true!=todo.empty())
	{
		...
		{
			auto neiPlHd=mesh.GetNeighborPolygon(plHd,i);
			if(mesh.GetColor(neiPlHd)!=col)
			{
				mesh.SetColor(neiPlHd,col);
...
int main(int ac,char *av[])
{
	...
	for(;;)
	{
		...
		if(0!=lb)
		{
			...
			{
				YsColor red;
				red.SetDoubleRGBA(1,0,0,1);
				mesh.SetColor(pickedPlHd,red);

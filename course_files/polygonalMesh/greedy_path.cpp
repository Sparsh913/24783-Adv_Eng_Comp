...
int main(int ac,char *av[])
{
	...
	for(;;)
	{
		...
		case FSKEY_X:
			...
			{
				auto pos=mesh.GetVertexPosition(selectedVtHd);
				pos.AddX(1.0);
				mesh.SetVertexPosition(selectedVtHd,pos);
		...
		if(FSMOUSEEVENT_LBUTTONDOWN==evt)
		{
			...
			if(mesh.NullVertex()!=pickedVtHd)
			{
				if(mesh.NullVertex()!=selectedVtHd)
				{
					for(auto vtHd : GreedyPathFinder(mesh,selectedVtHd,pickedVtHd))
					{
						auto pos=mesh.GetVertexPosition(vtHd);
						pathVtx.push_back(pos.x());
						pathVtx.push_back(pos.y());
						pathVtx.push_back(pos.z());
						pathCol.push_back(0);
						pathCol.push_back(0);
						pathCol.push_back(0);
						pathCol.push_back(1);

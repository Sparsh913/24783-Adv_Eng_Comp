...
int main(int ac,char *av[])
{
	...
	YsVec3 highlightPos[2]={YsVec3::Origin(),YsVec3::Origin()};
	PolygonalMesh::VertexHandle selectedVtHd[2]={mesh.NullVertex(),mesh.NullVertex()};
	...
	for(;;)
	{
		...
		{
		case FSKEY_C:
			if(mesh.NullVertex()!=selectedVtHd[0] && 
			   mesh.NullVertex()!=selectedVtHd[1])
			{ 
				CollapseEdge(mesh,selectedVtHd[0],selectedVtHd[1]);
		...
		case FSKEY_M:
			...
			break;
		case FSKEY_Q:
			// ApplyQEM_NSquare(mesh);
			{
				auto n=mesh.GetNumPolygons();
				while(n*9/10<mesh.GetNumPolygons())
				{
					qem.ApplyOne(mesh);
				}
			}
		...
		if(FSMOUSEEVENT_LBUTTONDOWN==evt)
		{
			...
			{
				selectedVtHd[0]=selectedVtHd[1];
				selectedVtHd[1]=pickedVtHd;
				highlightPos[0]=highlightPos[1];
				highlightPos[1]=mesh.GetVertexPosition(pickedVtHd);
			}
		}
		...
		glPointSize(5);
		glBegin(GL_POINTS);
		glVertex3dv(highlightPos[0]);
		glVertex3dv(highlightPos[1]);
		glEnd();

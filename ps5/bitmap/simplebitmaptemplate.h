...
public:
	...
	*/
	    ...
	    int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
		...
		for(int y=0; y<hei; ++y)
		{
			if(!IsYInRange(thisY0+y))
			{
				continue;
			}
			const int start_sx = std::max(0,thisX0);
			const int end_sx = std::min(thisX0+wid,nx);
			if(end_sx<=start_sx)
			{
				continue;
			}
			const int start_dx = start_sx-thisX0;
			const int num_pixels = end_sx-start_sx;

			const ComponentType *src = GetPixelPointer(start_sx,thisY0+y);
			ComponentType *dst = destination.GetEditablePixelPointer(start_dx,y);

			std::copy(src, src+num_pixels*NumComponentPerPixel,dst);
		}
	}

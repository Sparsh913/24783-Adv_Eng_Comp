...
void PolygonalMesh::Stitch(void)
{
	    ...
	    MoveToNext(plHd))
	{
		...
		{
			auto pos=GetVertexPosition(vtHd);
			auto idx0=ltc.GetBlockIndex(pos);
			for(int nei=0; nei<27; ++nei)
			{
				int dx=nei%3;
				int dy=(nei%9)/3;
				int dz=nei/9;
				...
				if(true==ltc.IsInRange(idx))
				{
					for(auto tst : ltc[idx])
					{
						if(GetSearchKey(tst)<GetSearchKey(vtHd) &&
						   GetVertexPosition(tst)==pos)
						{
							vtHd=tst;
							changed=true;
						}

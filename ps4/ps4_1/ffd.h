...
public:
	...
	*/
	...
	static std::vector <Point> FFDArrayToControlPoints(int n,const FreeFormDeformation box[]) // box is an array of FreeFormDeformation objects
	{
		std::vector <Point> ctp;
		if(0<n)
		{
			for(int i=0; i<n; ++i) // add to ctp the control points of the quadrilateral
			{
				ctp.push_back(box[i].quad[0]); //push back means adding an element to the end of the vector
				ctp.push_back(box[i].quad[1]);
			}
			ctp.push_back(box[n-1].quad[3]);
			ctp.push_back(box[n-1].quad[2]); //add the last control points of the quadrilateral
		}
		return ctp;
	}
